...
def read_dataset(path):
    ...
    x_train, x_test, y_train, y_test = train_test_split(dataset.text, dataset.spam, random_state = 0, test_size = 0.30)
    data = x_train.tolist() + x_test.tolist()
    x_train = label_sentences(x_train, 'Train')
    x_test = label_sentences(x_test, 'Train')
...
def train_doc2vec(corpus):
    ...
    d2v = doc2vec.Doc2Vec(min_count =1, window=3, vector_size = 100, dm =1, sample = 1e-3, seed =1, workers = 5)
...
if __name__ == "__main__":
    x_train, x_test, y_train, y_test, all = read_dataset('emails.csv')
